...
createCollege = async function (req, res) {
    try {
        ...
        if (Object.keys(rest).length > 0) {
            return res.status(400).send({ status: false, message: "Please Enter the Valid Attribute Field " });
        }

        //check the Name & Last fullName is present in req.body or not ?
        if (!name || !fullName) {

            return res.status(400).send({ status: false, message: "Missing Name or fullName" });
        }
        ...
        if (name.split(" ").length > 1) return res.status(400).send({ status: false, message: "please provide valid abbrevetion" })
        //var regName = /^[a-zA-Z]+/;
        ...
        if (nameFlag) {
            return res.status(400).send({ status: false, message: "name is Already Present in DB" })
        }
...
const getCollegeDetail = async function (req, res) {
    ...
    try {
        ...
        if (Object.keys(query).length === 0) {
            return res.status(400).send({ status: false, message: "pls Enter Query" })
        }

        //checkif valid quary is present or not
        if (Object.keys(rest).length > 0) {
            return res.status(400).send({ status: false, message: "pls Enter valid Query" })
        }
        ...
        if (!collegeDetail) {
            return res.status(404).send({ status: false, message: `${query.collegeName} this college is not present .` })
        }
        ...
        if (interns.length == 0) return res.status(404).send({ status: false, message: "No intern Found" });
