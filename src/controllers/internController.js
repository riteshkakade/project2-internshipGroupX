...
createIntern = async function (req, res) {
    try {
        ...
        if (!name) {
            return res.status(400).send({ status: false, message: "Missing name" });
        ...
        if (!mobile) {
            return res.status(400).send({ status: false, message: "Missing mobile" });

        }

        //check collegeName is present or not
        if (!collegeName) {
            return res.status(400).send({ status: false, message: "Missing collegeName" });
        ...
        if (!regName.test(mobile)) {
            //console.log(mobile)
            return res.status(400).send({ status: false, message: "mobile number invalid" });
        }

        //check if mobileNumber is present in Db or Not ? 
        let mobileNumber = await internModel.findOne({ mobile: mobile })
        if (mobileNumber) return res.status(400).send({ status: false, message: "This mobileNumber is already present in DB" })
